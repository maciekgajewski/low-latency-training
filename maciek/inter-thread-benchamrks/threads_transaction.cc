...
struct SharedData {
  std::atomic<std::uint64_t> version_written;
...
int main(int, char **) {
  ...
  for (int i = 0; i < CYCLES; ++i) {
    ...
    {
      //      std::uint64_t version = data.version;
      //      version++;
      // data.version = version;
      data.version_written++;
  ...
  while (true) {
    {
      ...
      version_written = data.version_written;
    }

    if (version != version_written) {
      retries++;
      continue;
    }
